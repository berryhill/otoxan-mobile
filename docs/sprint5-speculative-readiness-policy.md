...
5. No-fake-transcript behavior is non-negotiable. Empty STT must preserve an empty transcript, surface `Audio arrived, but speech did not decode. Try again after Listening appears.`, and must not send route metadata to Xander as if it were user speech.
